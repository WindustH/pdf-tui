...
mod tests {
  ...
  #[tokio::test]
  async fn clear_cache_removes_page_and_render_dirs_but_keeps_logs() -> Result<()> {
    let cache_dir = unique_temp_dir();
    fs::create_dir_all(cache_dir.join("pages").join("nested")).await?;
    fs::create_dir_all(cache_dir.join("render")).await?;
    fs::create_dir_all(cache_dir.join("logs")).await?;
    fs::write(cache_dir.join("pages").join("page.png"), [1_u8, 2, 3]).await?;
    fs::write(cache_dir.join("pages").join("page.toml"), [4_u8]).await?;
    fs::write(
      cache_dir.join("pages").join("nested").join("page.png.used"),
      [5_u8],
    )
    .await?;
    fs::write(cache_dir.join("render").join("render.ansi"), [6_u8, 7]).await?;
    fs::write(cache_dir.join("logs").join("current.log"), [8_u8]).await?;

    let report = clear_cache(&cache_dir).await?;

    assert_eq!(report.removed_files, 4);
    assert_eq!(report.removed_bytes, 7);
    assert!(fs::metadata(cache_dir.join("pages")).await?.is_dir());
    assert!(fs::metadata(cache_dir.join("render")).await?.is_dir());
    assert!(
      fs::read_dir(cache_dir.join("pages"))
        .await?
        .next_entry()
        .await?
        .is_none()
    ...
    assert!(
      fs::read_dir(cache_dir.join("render"))
        .await?
        .next_entry()
        .await?
        .is_none()
    ...
    assert_eq!(
      fs::read(cache_dir.join("logs").join("current.log")).await?,
      vec![8_u8]
    );
  ...
  fn unique_temp_dir() -> PathBuf {
    let nanos = SystemTime::now()
      ...
      .unwrap_or_default()
      .as_nanos();
    std::env::temp_dir().join(format!("pdf-tui-cache-test-{}-{nanos}", std::process::id()))
  }
}
